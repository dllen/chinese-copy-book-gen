(function(){
  ...
  function svgDataURL(type,size,color){ const s=size,c=color; if(type==='无格') return ''; if(type==='田字格'){ const svg=`<svg xmlns='http://www.w3.org/2000/svg' width='${s}' height='${s}'>`+`<rect x='0.5' y='0.5' width='${s-1}' height='${s-1}' fill='none' stroke='${c}' stroke-width='1'/>`+`<line x1='${s/2}' y1='1' x2='${s/2}' y2='${s-1}' stroke='${c}' stroke-width='1'/>`+`<line x1='1' y1='${s/2}' x2='${s-1}' y2='${s/2}' stroke='${c}' stroke-width='1'/>`+`</svg>`; return `url("data:image/svg+xml;utf8,${encodeURIComponent(svg)}")`; } if(type==='米字格'){ const svg=`<svg xmlns='http://www.w3.org/2000/svg' width='${s}' height='${s}'>`+`<rect x='0.5' y='0.5' width='${s-1}' height='${s-1}' fill='none' stroke='${c}' stroke-width='1'/>`+`<line x1='${s/2}' y1='1' x2='${s/2}' y2='${s-1}' stroke='${c}' stroke-width='1'/>`+`<line x1='1' y1='${s/2}' x2='${s-1}' y2='${s/2}' stroke='${c}' stroke-width='1'/>`+`<line x1='1' y1='1' x2='${s-1}' y2='${s-1}' stroke='${c}' stroke-width='1'/>`+`<line x1='${s-1}' y1='1' x2='1' y2='${s-1}' stroke='${c}' stroke-width='1'/>`+`</svg>`; return `url("data:image/svg+xml;utf8,${encodeURIComponent(svg)}")`; } if(type==='回宫格'){ const inner=Math.round(s*0.6),offset=(s-inner)/2; const svg=`<svg xmlns='http://www.w3.org/2000/svg' width='${s}' height='${s}'>`+`<rect x='0.5' y='0.5' width='${s-1}' height='${s-1}' fill='none' stroke='${c}' stroke-width='1'/>`+`<rect x='${offset}' y='${offset}' width='${inner}' height='${inner}' fill='none' stroke='${c}' stroke-width='1'/>`+`<line x1='${s/2}' y1='1' x2='${s/2}' y2='${s-1}' stroke='${c}' stroke-width='1'/>`+`<line x1='1' y1='${s/2}' x2='${s-1}' y2='${s/2}' stroke='${c}' stroke-width='1'/>`+`</svg>`; return `url("data:image/svg+xml;utf8,${encodeURIComponent(svg)}")`; } if(type==='四线三格'){ const y1=Math.round(s*0.2), y2=Math.round(s*0.5), y3=Math.round(s*0.75), y4=Math.round(s*0.92); const svg=`<svg xmlns='http://www.w3.org/2000/svg' width='${s}' height='${s}'>`+`<line x1='1' y1='${y1}' x2='${s-1}' y2='${y1}' stroke='${c}' stroke-width='1'/>`+`<line x1='1' y1='${y2}' x2='${s-1}' y2='${y2}' stroke='${c}' stroke-width='1' stroke-dasharray='4,4'/>`+`<line x1='1' y1='${y3}' x2='${s-1}' y2='${y3}' stroke='${c}' stroke-width='1'/>`+`<line x1='1' y1='${y4}' x2='${s-1}' y2='${y4}' stroke='${c}' stroke-width='1'/>`+`</svg>`; return `url("data:image/svg+xml;utf8,${encodeURIComponent(svg)}")`; } return ''; }
  ...
  function App(){
    ...
    useEffect(()=>{ if(feature==='数字字母'){ setGridType('四线三格'); if(!text||!text.trim()) setText('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'); } },[feature]);
    ...
    const parsed=useMemo(()=>{ if(feature==='控笔字帖'){ const basic=['一','丨','丿','丶','亅']; const mids=['氵','亻','讠','艹','月','女','口','木','火','土','日','目','田']; const adv=['永','德','善','爱','勇','强']; let pool=[]; if(difficulty==='初级') pool=basic; else if(difficulty==='中级') pool=mids; else pool=adv; const seq=[]; pool.forEach(c=>{ seq.push(c); seq.push(''); }); return { pages:[seq] }; } if(feature==='数字字母'){ const s=(text||'').replace(/[^0-9a-zA-Z]/g,''); const arr=Array.from(s); return { pages:[arr] }; } return toCells(mode,text,variant); },[feature,mode,text,variant,difficulty]);
